...
def uploadTaoTiao(videoBean):
    ...
    print("今日头条小视频上传成功")
    browser.find_element_by_xpath("//textarea[@class='tui2-input']").send_keys(
        videoBean.title)
...
def uploadQuanming(videoBean):
    ...
    sleep(3)
    browser.find_element_by_xpath("//span[@class='ant-upload ant-upload-btn']/input").send_keys(
        videoBean.videoPath)
    # 等待页面加载
    print("开始上传全民小视频")
    ...
    sleep(0.3)
    browser.find_element_by_xpath("//textarea[@class='ant-input']").send_keys(
        videoBean.title)
